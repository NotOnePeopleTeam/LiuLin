...
import { getMenuList, getUserInfo,phoneNumber} from '../../api/user.js';
...
Page({
  ...
  getPhoneNumber (e) {
    ...
    wx.getStorage({
      ...
      success(res){
        phoneNumber({
          wxphone: e.detail.encryptedData,iv:e.detail.iv, cache_key:res.data
        }).then(res=>{
          console.log( res);
          if(res.msg =="ok"){
            app.Tips({ title: '获取微信绑定手机号成功' });
            this.getUserInfo();
          }else{
            app.Tips({ title: '获取微信绑定手机号失败' });
          }
        })
      }
    });
