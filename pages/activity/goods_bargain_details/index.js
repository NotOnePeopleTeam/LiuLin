...
import {shareBind, getUserInfo} from '../../../api/user'
const app = getApp();
...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.scene) {
      ...
      getUserInfo().then(res => {
        shareBind({spread: app.globalData.spid, uid: res.data.uid}).then(res => {
          console.log(res);
        })
      });
    }
  ...
  setBargain:function(){//参与砍价
    ...
    postBargainStart(that.data.id).then(res=>{
      ...
      app.globalData.openPages = '/pages/activity/goods_bargain_details/index?id=' + this.data.id + '&bargain=' + this.data.bargainUid
